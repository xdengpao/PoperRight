...
- [x] 1.1 重构 `_write_to_postgresql` 为批量 INSERT
...
- [x] 2.1 改造 `TushareAdapter` 支持持久 httpx.AsyncClient
...
- [x] 2.2 在 `_process_import` 中确保 adapter 资源释放
...
- [x] 3.1 全局替换 `time.sleep` 为 `await asyncio.sleep`
...
- [x] 4.1 在 `_process_batched` 中新增失败项追踪和 batch_stats
  - 新增 `failed_codes: list[str]` 列表
  - 在 API 调用或 DB 写入异常时将 ts_code 追加到 failed_codes
  - 为 `_process_batched` 新增 batch_stats 返回结构（当前该函数不返回 batch_stats）
  - batch_stats 包含 `batch_mode`、`total_codes`、`failed_codes`（截取前 100 个）
  - _Requirements: 5.1_

- [x] 4.2 在 `_process_batched_by_date` 中新增失败区间追踪
  - 新增 `failed_chunks: list[str]` 列表
  - 在异常时将 `{chunk_start}-{chunk_end}` 追加到 failed_chunks
  - 在 batch_stats 中包含 `failed_chunks`（截取前 100 个）
  - _Requirements: 5.2_

- [x] 4.3 在 `_process_batched_index` 中新增失败项追踪和 batch_stats
  - 新增 `failed_codes: list[str]` 列表
  - 在异常时将 ts_code 追加到 failed_codes
  - 为 `_process_batched_index` 新增 batch_stats 返回结构（当前该函数不返回 batch_stats）
  - batch_stats 包含 `batch_mode`、`total_indices`、`failed_codes`（截取前 100 个）
  - _Requirements: 5.1_

- [x] 4.4 在 `_update_progress` 中传递 failed_items
  - 新增 `failed_items` 可选参数
  - 在 Redis 进度 JSON 中包含 `failed_items` 字段
  - _Requirements: 5.3_

- [x] 4.5 优化 `_get_stock_list` 代码后缀推断
...
- [x] 4.7 sector 成分导入日志增强
  - 在 `_process_batched_by_sector` 的 batch_stats 中新增 `trade_date` 字段（记录注入的日期）
  - _Requirements: 8.1, 8.2, 8.3_

- [x] 4.8 检查点 — 验证阶段 4
  - 确认 failed_codes / failed_chunks 在 batch_stats 中正确传递（含 by_code、by_date、by_index 三种模式）
