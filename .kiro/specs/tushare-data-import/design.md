...
本设计文档描述 Tushare 数据在线导入功能的技术架构和实现方案。该功能在现有 A 股量化交易系统的"数据管理 > 在线数据"菜单下新增"tushare"子菜单，提供 Tushare 平台 60+ 个 API 接口的数据导入能力，覆盖股票基础数据、行情数据（低频/中频）、财务数据、参考数据、特色数据、两融及转融通、资金流向、打板专题以及指数专题等全部数据分类。

### 设计目标

1. **全接口覆盖**：支持 Tushare 平台全部可用数据接口的在线导入，按"股票数据"和"指数专题"两大分类组织
2. **异步任务架构**：通过 Celery + Redis 实现异步导入，支持进度追踪和任务停止控制
3. **三级 Token 路由**：根据接口权限级别（basic/advanced/special）自动选择对应 Token
...
| 决策 | 选择 | 理由 |
|------|------|------|
| 导入编排模式 | 注册表驱动（API_Registry） | 60+ 接口逐个写 handler 不可维护，注册表模式统一字段映射和存储目标 |
...
| 板块数据 | 复用 sector_info/sector_constituent | 通过 data_source 枚举区分来源（THS/DC/CI），减少表数量 |
| Token 选择 | 配置层三级 Token + API_Registry 标注权限 | 运行时自动路由，缺失 Token 回退到默认 |
...
class TushareHealthResponse(BaseModel):
    connected: bool
    tokens: dict                     # {basic: {configured: bool}, ...}
...
// 页面加载时：
// 1. GET /data/tushare/health → 连接状态 + Token 配置
...
```python
# 新增三级 Token 配置
tushare_token_basic: str = ""       # 6000 积分及以下
tushare_token_advanced: str = ""    # 6000 积分以上
...
| 表名 | 引擎 | 复用场景 |
|------|------|---------|
| `kline` | TimescaleDB | 股票日K/周K/月K/分钟K + 指数日线/周线/月线/分钟 |
| `adjustment_factor` | TimescaleDB | 复权因子 |
| `stock_info` | PostgreSQL | stock_basic/fina_indicator/daily_basic/stk_delist/bak_basic 更新 |
| `sector_info` | PostgreSQL | index_classify/ths_index/dc_index（data_source 区分） |
| `sector_constituent` | PostgreSQL | ths_member/dc_member（data_source 区分） |
| `sector_kline` | TimescaleDB | sw_daily/ci_daily（data_source 区分） |
...
以下表结构遵循相同模式（PGBase、Mapped[]、UniqueConstraint），按需求 25 验收标准逐一创建：

| 表名 | 主要字段 | 唯一约束 |
|------|---------|---------|
| `suspend_info` | ts_code, suspend_date, resume_date, suspend_type | — |
| `stock_company` | ts_code(PK), chairman, manager, secretary, reg_capital, setup_date, province, city, website | ts_code |
| `stock_namechange` | ts_code, name, start_date, end_date, change_reason | — |
| `hs_constituent` | ts_code, hs_type, in_date, out_date, is_new | — |
| `stk_rewards` | ts_code, ann_date, name, title, reward, hold_vol | — |
| `stk_managers` | ts_code, ann_date, name, gender, lev, title, edu, national, birthday, begin_date, end_date | — |
| `margin_data` | trade_date, exchange_id, rzye, rzmre, rzche, rqye, rqmcl, rzrqye | (trade_date, exchange_id) |
| `margin_detail` | ts_code, trade_date, rzye, rzmre, rzche, rqye, rqmcl, rqyl | (ts_code, trade_date) |
| `margin_target` | ts_code, mg_type, is_new | — |
| `slb_len` | ts_code, trade_date, len_rate, len_amt | — |
| `slb_sec` | ts_code, trade_date, sec_amt, sec_vol | — |
| `moneyflow_hsgt` | trade_date, ggt_ss, ggt_sz, hgt, sgt, north_money, south_money | (trade_date) |
| `moneyflow_ind` | trade_date, industry_name, data_source, buy_amount, sell_amount, net_amount | — |
| `moneyflow_mkt_dc` | trade_date, close, change, pct_change, net_mf_amount, net_mf_amount_rate, buy_elg_amount, sell_elg_amount, buy_lg_amount, sell_lg_amount, buy_md_amount, sell_md_amount, buy_sm_amount, sell_sm_amount | (trade_date) |
| `block_trade` | ts_code, trade_date, price, vol, amount, buyer, seller | (ts_code, trade_date, buyer, seller) |
| `top_holders` | ts_code, ann_date, end_date, holder_name, hold_amount, hold_ratio, holder_type | (ts_code, end_date, holder_name, holder_type) |
| `stk_holdernumber` | ts_code, ann_date, end_date, holder_num, holder_num_change | — |
| `stk_holdertrade` | ts_code, ann_date, holder_name, change_vol, change_ratio, after_vol, after_ratio, in_de | — |
| `stk_account` | date, weekly_new, total, weekly_hold | — |
| `stk_limit` | ts_code, trade_date, up_limit, down_limit | (ts_code, trade_date) |
| `stk_factor` | ts_code, trade_date, close, macd_dif, macd_dea, macd, kdj_k, kdj_d, kdj_j, rsi_6, rsi_12, rsi_24, boll_upper, boll_mid, boll_lower, cci | (ts_code, trade_date) |
| `limit_list` | ts_code, trade_date, industry, close, pct_chg, amount, limit_amount, float_mv, total_mv, turnover_ratio, fd_amount, first_time, last_time, open_times, up_stat, limit_times, limit | (ts_code, trade_date, limit) |
| `limit_step` | ts_code, trade_date, name, close, pct_chg, step, limit_order, amount, turnover_ratio, fd_amount, first_time, last_time, open_times | (ts_code, trade_date) |
| `hm_list` | hm_name, hm_code, market, desc | — |
| `hm_detail` | trade_date, ts_code, hm_name, buy_amount, sell_amount, net_amount | — |
| `top_list` | trade_date, ts_code, name, close, pct_change, turnover_rate, amount, l_sell, l_buy, l_amount, net_amount, net_rate, amount_rate, float_values, reason | (trade_date, ts_code, reason) |
| `top_inst` | trade_date, ts_code, exalter, buy, buy_rate, sell, sell_rate, net_buy | (trade_date, ts_code, exalter) |
| `ths_limit` | trade_date, ts_code, name, close, pct_chg, fd_amount, first_time, last_time, open_times, limit | (ts_code, trade_date, limit) |
| `dc_hot` | trade_date, ts_code, name, rank, pct_chg, hot_value | — |
| `ths_hot` | trade_date, ts_code, name, rank, pct_chg, hot_value | — |
| `kpl_list` | trade_date, ts_code, name, close, pct_chg, tag | — |
| `index_global` | ts_code, trade_date, open, close, high, low, pre_close, change, pct_chg, vol, amount | (ts_code, trade_date) |
| `index_tech` | ts_code, trade_date, close, macd_dif, macd_dea, macd, kdj_k, kdj_d, kdj_j, rsi_6, rsi_12, boll_upper, boll_mid, boll_lower | (ts_code, trade_date) |
| `market_daily_info` | trade_date, exchange, ts_code, ts_name, com_count, total_share, float_share, total_mv, float_mv, amount, vol, trans_count | (trade_date, exchange, ts_code) |
| `sz_daily_info` | trade_date, ts_code, count, amount, vol, total_share, total_mv, float_share, float_mv | (trade_date, ts_code) |
| `new_share` | ts_code(PK), sub_code, name, ipo_date, issue_date, amount, market_amount, price, pe, limit_amount, funds, ballot | ts_code |
| `stock_st` | ts_code, name, is_st, st_date, st_type | — |
| `daily_share` | ts_code, trade_date, total_share, float_share, free_share, total_mv, float_mv | (ts_code, trade_date) |
...
```bash
alembic revision --autogenerate -m "add tushare import tables"
...
*For any* valid Tushare ts_code 字符串（格式为 "XXXXXX.XX"，如 "600000.SH"、"000001.SZ"），将其转换为纯 6 位 symbol 时，结果应等于 ts_code 中点号之前的部分，且长度为 6。
...
*For any* TokenTier（basic/advanced/special）和任意 Token 配置组合，`_resolve_token` 应：(a) 当对应级别 Token 已配置时返回该 Token；(b) 当对应级别 Token 为空但默认 Token 已配置时返回默认 Token；(c) 当两者均为空时抛出错误。

**Validates: Requirements 22a.3, 22a.4**

### Property 5: API_Registry 条目完整性

*For any* ApiEntry 在 TUSHARE_API_REGISTRY 中，其 `token_tier` 应为 {BASIC, ADVANCED, SPECIAL} 之一，`code_format` 应为 {STOCK_SYMBOL, INDEX_CODE, NONE} 之一，`storage_engine` 应为 {PG, TS} 之一，且 `target_table` 不为空。
...
*For any* ApiEntry，TushareImportView 渲染的参数表单应包含该 entry 的 `required_params` 和 `optional_params` 中声明的所有参数类型对应的 UI 控件（DATE_RANGE → 日期选择器，STOCK_CODE → 代码输入框，MARKET → 市场下拉框，FREQ → 频率选择器，REPORT_PERIOD → 报告期选择器）。
...
| Property 4 | Token 路由与回退 | 随机 TokenTier + 随机 Token 配置组合 |
| Property 5 | Registry 条目完整性 | 遍历全部注册条目 |
...
| Property 11 | Token 不可用禁用 | 随机 token_tier + 随机 token 配置 |
...
| 测试文件 | 覆盖范围 |
|---------|---------|
| `tests/services/test_tushare_import_service.py` | Import_Service 参数校验、Token 路由、任务分发 |
| `tests/services/test_tushare_registry.py` | API_Registry 注册、查询、分类 |
