...
graph LR
    ...
    subgraph After["重构后"]
        DS2["DataSource 主维度（Level 1）<br/>DC/THS/TDX/TI/CI"]
        ST2["SectorType 可选过滤（Level 2）<br/>str | None，默认 None<br/>有效值由 sector_info 表动态决定"]
    end

    Before -->|重构| After

    subgraph API["新增 API"]
        TYPES["GET /api/v1/sector/types<br/>?data_source={ds}<br/>返回可用 sector_type + 中文标签"]
        COV["GET /api/v1/sector/coverage<br/>增加 type_breakdown 字段"]
    end
...
```python
# 旧枚举值集合，用于 from_dict() 向后兼容
_OLD_SECTOR_TYPE_ENUMS = {"CONCEPT", "INDUSTRY", "REGION", "STYLE"}

@dataclass
class SectorScreenConfig:
    sector_data_source: str = "DC"       # 主维度（Level 1）：DC/THS/TDX/TI/CI
    sector_type: str | None = None       # 可选二级过滤（Level 2），默认 None 查询全部
    sector_period: int = 5
    sector_top_n: int = 30

    def to_dict(self) -> dict:
        """序列化为字典。sector_type 仅在非 None 时输出。"""
        d = {
        ...
        if self.sector_type is not None:
            d["sector_type"] = self.sector_type
        return d

    @classmethod
    def from_dict(cls, data: dict) -> "SectorScreenConfig":
        """反序列化。向后兼容旧配置中的 sector_type 枚举值。"""
        raw_sector_type = data.get("sector_type")
        # 旧枚举值（CONCEPT/INDUSTRY/REGION/STYLE）→ 忽略，设为 None
        if raw_sector_type in _OLD_SECTOR_TYPE_ENUMS:
            sector_type = None
        else:
            sector_type = raw_sector_type  # None 或有效的新字符串值
        return cls(
...
**`SECTOR_TYPE_LABEL_MAP` 常量**（需求 22.9）：

```python
SECTOR_TYPE_LABEL_MAP: dict[str, str] = {
    # THS（同花顺）
    "I": "行业板块",
    "N": "概念板块",
    "R": "地域板块",
...
def get_sector_type_label(raw_type: str | None) -> str:
    """获取 sector_type 的中文显示标签。None 返回"未分类"，未知值返回原始值。"""
    if raw_type is None:
        return "未分类"
    return SECTOR_TYPE_LABEL_MAP.get(raw_type, raw_type)
```

**新增 API 端点**（需求 22.8）：`GET /api/v1/sector/types?data_source={ds}`
...
**覆盖率 API 增强**（需求 22.10）：`GET /api/v1/sector/coverage` 响应中每个数据来源新增 `type_breakdown` 字段：

```json
{
    "data_source": "THS",
    "total_sectors": 850,
    "sectors_with_constituents": 820,
    "total_stocks": 4500,
    "coverage_ratio": 0.965,
    "type_breakdown": [
        {"sector_type": "I", "label": "行业板块", "sector_count": 120, "stock_count": 3800},
        {"sector_type": "N", "label": "概念板块", "sector_count": 500, "stock_count": 4200},
        {"sector_type": "R", "label": "地域板块", "sector_count": 31, "stock_count": 2500}
    ]
}
```
...
**现有策略示例更新**（需求 22.14）：
现有 12 个策略示例中，示例 3（概念板块热点龙头）、4（行业板块轮动策略）、8（板块强势+布林突破）、10（概念板块+形态突破联动）、11（多数据源板块交叉验证）、12（RSI超卖反弹+板块支撑）的 `sector_config` 需移除 `sector_type` 字段，仅保留 `sector_data_source`、`sector_period`、`sector_top_n`（`sector_type` 默认为 `None` 时 `to_dict()` 不输出该字段）。
...
- 板块面因子配置区域展示两个并排下拉框：
  - 第一个下拉框"数据来源"（`sector_data_source`），选项为 DC/THS/TDX/TI/CI，每个选项附带覆盖率统计（格式如"东方财富 DC (1020 板块 / 5595 股票)"）
  - 第二个下拉框"板块类型"（`sector_type`），根据第一个下拉框选中的数据来源动态填充选项，首选项为"全部"（对应 `sector_type=None`）。每个选项显示中文名称 + 原始代码，格式如"行业板块 (I)"或"一级行业 (L1)"
- 用户切换数据来源时，前端重置板块类型下拉为"全部"，并调用 `GET /api/v1/sector/types?data_source={ds}` 动态填充选项
- `factorNameOptions` 按新类别分组

**`screener.ts` store 变更**：
- `FactorMeta` 接口不变（已支持所有字段）
- `SectorScreenConfig` 接口保留 `sector_type` 为可选字段（`string | null`），`sector_data_source` 为必选
...
### 7. API 层变更（`screen.py` / `sector.py`）

- `get_strategy_examples()` 端点返回 `config_doc` 字段
- `SectorScreenConfigIn` 模型保留 `sector_type` 为可选字段（`str | None = None`）
- 因子注册表 API 支持新增类别查询
- **新增** `GET /api/v1/sector/types?data_source={ds}` 端点（需求 22.8）：返回指定数据来源下可用的 `sector_type` 值列表，每个值附带中文标签（`label`）和板块数量（`count`）
- **增强** `GET /api/v1/sector/coverage` 端点（需求 22.10）：每个数据来源的统计中新增 `type_breakdown` 字段，按 `sector_type` 分组返回板块数量和成分股数量，每个分组附带中文标签
...
```
`sector_type` 仅在非 `None` 时条件性输出。

**`from_dict()` 输入兼容**（需求 22.12）：
- 旧配置含旧枚举值 `sector_type`（如 `CONCEPT`、`INDUSTRY`）→ 忽略，设为 `None`
- 旧配置不含 `sector_type` → 默认 `None`
- 新配置含有效 `sector_type` 字符串（如 `"I"`、`"L1"`）→ 正确读取
- 旧配置含 `sector_data_source` → 正常读取
- 无 `sector_data_source` → 默认 `"DC"`

### SECTOR_TYPE_LABEL_MAP 映射表

后端维护的 `sector_type` 原始值到中文名称的映射常量（需求 22.9），位于 `app/core/schemas.py` 或 `app/api/v1/sector.py`：
...
**Validates: Requirements 18.5, 21.6, 22.12**

### Property 9: SectorScreenConfig 序列化条件性包含 sector_type

*For any* `SectorScreenConfig` 实例，`to_dict()` 的输出字典应满足：
1. 当 `sector_type` 为 `None` 时，输出字典不包含 `sector_type` 键
2. 当 `sector_type` 为非 `None` 字符串时，输出字典包含 `sector_type` 键且值等于该字符串
3. `from_dict()` 对包含旧枚举值（`CONCEPT`、`INDUSTRY`、`REGION`、`STYLE`）的配置字典不报错，且将 `sector_type` 设为 `None`
4. `from_dict()` 对包含有效新字符串值（如 `"I"`、`"L1"`）的配置字典能正确读取 `sector_type`
5. `from_dict(to_dict(config))` 应产生与原始配置等价的实例（往返一致性）
...
*For any* `SECTOR_TYPE_LABEL_MAP` 中的键值对，`get_sector_type_label(key)` 应返回对应的中文标签值。*For any* 不在 `SECTOR_TYPE_LABEL_MAP` 中的字符串 `s`，`get_sector_type_label(s)` 应返回 `s` 本身（回退到原始值）。
...
| 测试类型 | 覆盖范围 | 文件位置 |
|----------|----------|----------|
| 属性测试 | 17 个正确性属性 | `tests/properties/test_screening_enhancement_props.py` |
...
| 单元测试 - 板块类型 API | sector/types 端点 + SECTOR_TYPE_LABEL_MAP | `tests/api/test_sector_api.py` |
...
4. **SectorScreenConfig 向后兼容**：用包含旧枚举值 `sector_type`（如 `CONCEPT`）的旧配置测试 `from_dict()` 设为 `None`；用包含有效新值（如 `"I"`）的配置测试正确读取；用 `sector_type=None` 和非 `None` 的实例测试 `to_dict()` 条件性输出
